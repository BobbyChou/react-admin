...
import { MessageOutlined, BookOutlined, DownOutlined } from "@ant-design/icons";
...
const Layout = (props: any) => {
  ...
  useEffect(() => {
    window.onhashchange = function (event: any) {
            ...
            return (
              <Menu.Item key={`${item}`}>
                <div
                  ...
                  onClick={() => {
                    props.dispatch(setWebTheme(item));
                  }}
                ></div>
              </Menu.Item>
            );
  ...
  const renderDocumentMenu = () => {
    ...
    return (
      <Fragment>
        {!!documentsMenu &&
          documentsMenu.map((item: any) => {
            return (
              <Menu.Item key={`${item?.id}`}>
                <Link
                  to={{ pathname: "/documents", component: item?.component }}
                >
                  {item?.name}
                </Link>
              </Menu.Item>
